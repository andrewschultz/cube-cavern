...
carry out verbing:
	say "You can move in any of the standard directions, e.g. [b]U/D/N/S/E/W[r]. [b]IN[r] also works if and when you have passage into the center of the cube.[paragraph break]On the [mrlp], you can make diagonal movements like [b][face-dirs][r].[paragraph break]You may also want to [b]TOUCH[r] things or [b]SUMMON[r] the four elements: [list of elements][if all-4-acc is true]. Or you can just type the element or color you want[end if][if c-known is true].[paragraph break][b]C (color)[r] changes the ring color and touches a beacon[end if].[paragraph break][b]THINK[r] or [b]HELP[r] or [b]HINT[r] will summarize where you've been and what you've done[if rope-drop is true and tunnel-looped is false]. [b]RESET[r] will send you back before when you pitched the rope[end if]. You can also GO TO/GT any location on the cube, in abbreviated form (e.g. UNW goes to the northwest corner of the upper face)[if path-in-verbs is true]. You can type [b]PATH[r] to toggle path tracking, if you are trying to touch every outside square once before moving on[end if].[paragraph break]If visualization is tricky, [b]READ MAP[r] gives a textual representation of the PDF map included with the game.";
	if thoughtfulness > 3, say "[line break][b]EXP[r] will also reveal what your most recent wandering thought or random find meant, if you didn't quite get the joke.";
	if debug-state is true:
